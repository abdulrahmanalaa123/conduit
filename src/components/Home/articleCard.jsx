...
function ArticleCard({ article }) {
  ...
  const favoriteArticle = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["global", "your", "tagged"]);
    },
  });
  ...
  function handleFavoriteClick() {
    if (liked) {
      favoriteArticle.mutate({ slug: article.slug });
    } else {
      unfavoriteArticle.mutate({ slug: article.slug });
    }
  }
      ...
      <div className="flex flex-row justify-between mb-2">
        ...
        >
          <svg
            ...
            viewBox="0 0 512 512"
            className={`${
              liked ? "fill-accentColor group-hover:fill-white" : "fill-white"
